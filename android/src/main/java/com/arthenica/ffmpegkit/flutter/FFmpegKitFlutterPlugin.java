 ...
 */
 ...
 public class FFmpegKitFlutterPlugin implements
         FlutterPlugin,
         ActivityAware,
         MethodCallHandler,
 ...
 sessionHistorySize != null) {
     ...
     protected void abstractSessionGetAllLogs(@NonNull final Integer sessionId, @Nullable final Integer waitTimeout, @NonNull final Result result) {
         ...
         if (session == null) {
             ...
             final int timeout = isValidPositiveNumber(waitTimeout)
                     ? waitTimeout : AbstractSession.DEFAULT_TIMEOUT_FOR_ASYNCHRONOUS_MESSAGES_IN_TRANSMIT;
     ...
     protected void getMediaInformation(@NonNull final Integer sessionId, @NonNull final Result result) {
         ...
         if (session == null) {
             ...
             final MediaInformationSession miSession = (MediaInformationSession) session;
             final MediaInformation mediaInformation = miSession.getMediaInformation();
     ...
     protected void ffmpegSessionExecute(@NonNull final Integer sessionId, @NonNull final Result result) {
         ...
         if (session == null) {
             ...
             final FFmpegSessionExecuteTask task = new FFmpegSessionExecuteTask((FFmpegSession) session, resultHandler, result);
             asyncExecutorService.submit(task);
         } else {
             resultHandler.errorAsync(result, "NOT_FFMPEG_SESSION", "A session is found but it does not have the correct type.");
         }
     }
     ...
     protected void ffprobeSessionExecute(@NonNull final Integer sessionId, @NonNull final Result result) {
        ...
        if (session == null) {
            ...
            final FFprobeSessionExecuteTask task = new FFprobeSessionExecuteTask((FFprobeSession) session, resultHandler, result);
            asyncExecutorService.submit(task);
        } else {
     ...
     protected void mediaInformationSessionExecute(@NonNull final Integer sessionId, @Nullable final Integer waitTimeout, @NonNull final Result result) {
         ...
         if (session == null) {
             ...
             final int timeout = isValidPositiveNumber(waitTimeout)
                     ? waitTimeout : AbstractSession.DEFAULT_TIMEOUT_FOR_ASYNCHRONOUS_MESSAGES_IN_TRANSMIT;
             final MediaInformationSessionExecuteTask task = new MediaInformationSessionExecuteTask((MediaInformationSession) session, timeout, resultHandler, result);
             asyncExecutorService.submit(task);
         } else {
             resultHandler.errorAsync(result, "NOT_MEDIA_INFORMATION_SESSION", "A session is found but it does not have the correct type.");
         }
     ...
     protected void writeToPipe(@NonNull final String inputPath, @NonNull final String namedPipePath, @NonNull final Result result) {
         final WriteToPipeTask task = new WriteToPipeTask(inputPath, namedPipePath, resultHandler, result);
         asyncExecutorService.submit(task);
     }
 
     protected void selectDocument(@NonNull final Boolean writable,
                                   @Nullable final String title,
                                   @Nullable final String type,
                                   @Nullable final String[] extraTypes,
         ...
         if (extraTypes != null) intent.putExtra(Intent.EXTRA_MIME_TYPES, extraTypes);
         ...
         if (context != null && activity != null) {
             try {
                 ...
                 Log.i(LIBRARY_NAME, String.format("Failed to selectDocument using parameters writable: %s, type: %s, title: %s and extra types: %s!",
             ...
             Log.w(LIBRARY_NAME, String.format("Cannot selectDocument; context or activity is null."));
             resultHandler.errorAsync(result, "INVALID_CONTEXT", "Context or activity is null.");
         }
     }
     ...
     protected void getSafParameter(@NonNull final String uriString, @NonNull final String openMode, @NonNull final Result result) {
         ...
         if (context != null) {
             ...
             if (uri == null) {
                 Log.w(LIBRARY_NAME, String.format("Cannot getSafParameter; uriString: %s could not be parsed.", uriString));
                 ...
                 Log.d(LIBRARY_NAME, String.format("getSafParameter completed with saf parameter: %s.", safParameter));
                 resultHandler.successAsync(result, safParameter);
             }
         } else {
             Log.w(LIBRARY_NAME, "Cannot getSafParameter; context is null.");
             resultHandler.errorAsync(result, "INVALID_CONTEXT", "Context is null.");
         }
     }
     ...
     protected static int toInt(final LogRedirectionStrategy strategy) {
         switch (strategy) {
             case ALWAYS_PRINT_LOGS: return 0;
             case PRINT_LOGS_WHEN_NO_CALLBACKS_DEFINED: return 1;
             case PRINT_LOGS_WHEN_GLOBAL_CALLBACK_NOT_DEFINED: return 2;
             case PRINT_LOGS_WHEN_SESSION_CALLBACK_NOT_DEFINED: return 3;
     ...
     protected static LogRedirectionStrategy toLogRedirectionStrategy(final int value) {
         switch (value) {
             case 0: return LogRedirectionStrategy.ALWAYS_PRINT_LOGS;
             case 1: return LogRedirectionStrategy.PRINT_LOGS_WHEN_NO_CALLBACKS_DEFINED;
             case 2: return LogRedirectionStrategy.PRINT_LOGS_WHEN_GLOBAL_CALLBACK_NOT_DEFINED;
             case 3: return LogRedirectionStrategy.PRINT_LOGS_WHEN_SESSION_CALLBACK_NOT_DEFINED;
             case 4:
             default: return LogRedirectionStrategy.NEVER_PRINT_LOGS;
         }
     }
     ...
     protected static SessionState toSessionState(final int value) {
         switch (value) {
             ...
             case 1: return SessionState.RUNNING;
             case 2: return SessionState.FAILED;
             case 3:
             default: return SessionState.COMPLETED;
         }
     }
     ...
     protected static Map<String, Object> toMap(final Statistics statistics) {
         ...
         if (statistics != null) {
             statsMap.put(KEY_STATISTICS_SESSION_ID, statistics.getSessionId());
             statsMap.put(KEY_STATISTICS_VIDEO_FRAME_NUMBER, statistics.getVideoFrameNumber());
             statsMap.put(KEY_STATISTICS_VIDEO_FPS, statistics.getVideoFps());
             statsMap.put(KEY_STATISTICS_VIDEO_QUALITY, statistics.getVideoQuality());
             statsMap.put(KEY_STATISTICS_SIZE,
                     (statistics.getSize() < Integer.MAX_VALUE)
                     ? (int) statistics.getSize()
                     : (int) (statistics.getSize() % Integer.MAX_VALUE));
             statsMap.put(KEY_STATISTICS_TIME, statistics.getTime());
             statsMap.put(KEY_STATISTICS_BITRATE, statistics.getBitrate());
             statsMap.put(KEY_STATISTICS_SPEED, statistics.getSpeed());
         }
         return statsMap;
     }
 
     protected static Map<String, Object> toMap(final MediaInformation mediaInformation) {
         if (mediaInformation == null) return null;
         JSONObject allProps = mediaInformation.getAllProperties();
         return (allProps != null) ? toMap(allProps) : new HashMap<>();
     }
     ...
     protected static List<Map<String, Object>> toSessionMapList(final List<? extends Session> sessions) {
     ...
     protected static List<Map<String, Object>> toLogMapList(final List<com.arthenica.ffmpegkit.Log> logs) {
         final List<Map<String, Object>> list = new ArrayList<>();
     ...
     protected static List<Map<String, Object>> toStatisticsMapList(final List<Statistics> stats) {
         final List<Map<String, Object>> list = new ArrayList<>();
     ...
     protected void emitLog(final com.arthenica.ffmpegkit.Log log) {
         final Map<String, Object> map = new HashMap<>();
         map.put(EVENT_LOG_CALLBACK_EVENT, toMap(log));
         resultHandler.successAsync(eventSink, map);
     }
 
     protected void emitStatistics(final Statistics statistics) {
         final Map<String, Object> map = new HashMap<>();
         map.put(EVENT_STATISTICS_CALLBACK_EVENT, toMap(statistics));
         resultHandler.successAsync(eventSink, map);
     }
 
     protected void emitSession(final Session session) {
         final Map<String, Object> map = new HashMap<>();
         map.put(EVENT_COMPLETE_CALLBACK_EVENT, toMap(session));
         resultHandler.successAsync(eventSink, map);
     }
 }
